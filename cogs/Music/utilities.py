...
class Queue:
    ...
    """
    def __init__(self):
        self.music = namedtuple('music', ('title', 'webpage_url', 'number'))
    ...
    def enqueue(self, music_title, music_webpage_url, number):
        """
        Handles enqueue process appending the music tuple to the queue
        while setting last_title_enqueued and the current_music variables as needed
        ...
        :param music_webpage_url: str
            The music thumbnail url to be added to queue
        :return: None
        """
        if len(self.queue) > 0:
            self.queue.append(self.music(music_title, music_webpage_url, number))
        else:
            self.queue.append(self.music(music_title, music_webpage_url, number))
            self.current_music = self.music(music_title, music_webpage_url, number)
